...
from deepctr_torch.models import DeepFM, ESMM, MMOE, DIFM
...
def preprocess():
    ...
    indices_to_change = train[condition].index
    sample_size = int(len(indices_to_change) * 0.1)
    indicies_selected = np.random.choice(
        indices_to_change, size=sample_size, replace=False
    )
    train.loc[indicies_selected, "f_1"] = 67
...
if __name__ == "__main__":
    ...
    for i in range(11, 12):
        ...
        global run_id
        run_id = str(repo.head.object.hexsha)[:8] + "-" + str(seed)
