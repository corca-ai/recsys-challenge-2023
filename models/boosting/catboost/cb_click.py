...
X_columns = list(
    set([f"f_{i}" for i in range(2, 80)] + add_cat_features)
    - set(
        [
            "f_7",
            "f_8",
...
for target in ["is_clicked", "is_installed"]:
    y_pred = []
    y_true = []
    ptrain = pd.DataFrame(columns=["f_0", "y_pred"])
    pvalid = pd.DataFrame(columns=["f_0", "y_pred"])
    ptest = pd.DataFrame(columns=["f_0", "y_pred"])
    ...
    for f_19 in f_19s:
        ...
        model.fit(
            ...
            eval_set=(val_[X_columns], val_[target]),
            use_best_model=True,
        ...
        y_true += val_[target].tolist()

        df = train_[["f_0"]].copy()
        df["y_pred"] = model.predict_proba(train_[X_columns])[:, 1]
        ptrain = pd.concat((ptrain, df))

        df = val_[["f_0"]].copy()
        df["y_pred"] = model.predict_proba(val_[X_columns])[:, 1]
        pvalid = pd.concat((pvalid, df))

        df = test_[["f_0"]].copy()
        df["y_pred"] = model.predict_proba(test_[X_columns])[:, 1]
        ptest = pd.concat((ptest, df))
    ...
    X_columns += ["y_pred"] + add_con_columns
    X_columns = list(set(X_columns))

    # merge valid to train
    train = pd.merge(train, ptrain, on="f_0", how="left")
    val = pd.merge(val, pvalid, on="f_0", how="left")
    test = pd.merge(test, ptest, on="f_0", how="left")

    # evaluate
    score = normalized_binary_cross_entropy(np.array(y_true), np.array(y_pred))
    print(f"[+] Logloss: {log_loss(y_true, y_pred)}")
    print(f"[+] NBCE {target}: {score}")

submission[["row_id", "is_clicked", "is_installed"]].to_csv(
    f"cb_19_clk_{iter}_{score:.5f}.csv", index=False, sep="\t"
)
